...
# Logan- function for easy drawing perfect square (1 same side len)
...
# Logan- function for easy drawing rectangle
...
# Logan- function to easily setup turtle in one line with common things to be set
...
def x_check(apple, snake):
  # Checks coordinates
  if (apple.xcor() - 15 < snake.xcor() < apple.xcor() + 15):
    return True
  else:
    return False
...
def y_check(apple, snake):
  # Checks coordinates
  if (apple.ycor() - 15 < snake.ycor() < apple.ycor() + 15):
    return True
  else:
    return False
