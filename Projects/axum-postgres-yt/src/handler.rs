...
// get all notes and return as json
pub async fn note_list_handler(
    ...
    if query_result.is_err() {
        ...
        return Err((StatusCode::INTERNAL_SERVER_ERROR, Json(error_respose)));
    }

    let notes = query_result.unwrap();
    let notes_len = notes.len();

    println!("-- Get Notes Called, returned {notes_len} notes.");
...
pub async fn get_note_handler(
    ...
    match query_result {
        Ok(note) => {
            let note_response = serde_json::json!({"status": "success", "data": serde_json::json!({"note": note})});
